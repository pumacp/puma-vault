---
id: VID-PM-001
title: "El viejo Product Manager ha muerto: bienvenido el IA PM"
type: video-note
channel: "Javier Garzás"
url: "https://www.youtube.com/watch?v=8Rcnj_JsZ84"
year: 2025
status: to-watch
tags: [video, pm, product-manager, ai, agile, academic-writing, backlog, code-review, github, issue-triage, llm, moc, planning, project-management, smart-pmo, sprint, triage, video-note]
phase: ['F0', 'F5']
rating: 5
created: 2026-04-06
updated: 2026-04-06
---

# 🎬 El viejo Product Manager ha muerto: bienvenido el IA PM

> [!info] Video Details
> **Channel**: Javier Garzás  
> **URL**: https://www.youtube.com/watch?v=8Rcnj_JsZ84  
> **Relevance**: ⭐⭐⭐⭐⭐

---

## Summary

Javier Garzás — Spain's most prominent Agile thought leader — argues that the traditional Product Manager role is being fundamentally transformed by AI. The AI PM: automated backlog triage and prioritisation (AI handles routine classification), data-driven sprint planning (AI analyses velocity and risk), and human PM focusing on stakeholder management and strategic decisions.

---

## PUMA Relevance

This video is the practitioner-level validation of PUMA's project: the shift from manual triage/estimation (old PM) to AI-automated triage/estimation (new PM) is not just theoretical but actively happening in industry. Can be referenced in PUMA Section 1.1 as evidence of industrial relevance alongside academic citations (Assalaarachchi 2026, Cinkusz 2025).

---

## Related Notes

- [[20 - Literature/20.1 Papers/PM-AI-Convergence/LN-Assalaarachchi-2026-AgenticSPM]]
- [[40 - Projects/PUMA/41.1 Introduction/PR-PUMA-Ch1-Introduction]]
- [[50 - Areas/56 Smart-PMO/Smart-PMO-Vision]]

## MOCs

- [[80 - MOC/81 Topic-Maps/MOC-PUMA-Master]]
- [[MOC-LLM-Benchmarks-PM-AI]]
