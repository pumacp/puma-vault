---
id: PT-P1S2-003
title: "OpenCode — Literature Synthesis with RCOIF Framework"
type: prompt-note
tool: claude
...
Context: I am developing PUMA (Project Understanding and Management with Agents), a open-source project following Design Science Research methodology. PUMA evaluates local LLM agents (Llama 3.2 8B, Mistral 7B via Ollama) for two tasks: (1) issue triage on Jira Social Repository dataset (F1-macro target ≥ 0.55) and (2) effort estimation on TAWOS dataset (MAE target ≤ 3.0 SP). All experiments use seed=42, temperature=0.0 for reproducibility.
